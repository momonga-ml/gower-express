...
def gower_matrix(data_x, data_y=None, weight=None, cat_features=None, n_jobs=1, use_gpu=False):  
    """
    ...
    if use_gpu and GPU_AVAILABLE:
        ...
        try:
            ...
            out_gpu = gower_matrix_vectorized_gpu(
                X_cat_gpu, X_num_gpu, Y_cat_gpu, Y_num_gpu,
                weight_cat_gpu, weight_num_gpu, weight_sum,
                num_ranges_gpu, x_n_rows == y_n_rows, xp
            )
...
def gower_matrix_vectorized_gpu(X_cat, X_num, Y_cat, Y_num, weight_cat, weight_num, 
                               weight_sum, num_ranges, is_symmetric, xp):
    """
    ...
    if X_num.shape[1] > 0:
        ...
        # Normalize by ranges
        normalized_delta = xp.divide(abs_delta, num_ranges[xp.newaxis, xp.newaxis, :],
...
def gower_topn(data_x, data_y=None, weight=None, cat_features=None, n=5, use_optimized=True):
    """
...
def gower_topn_optimized(data_x, data_y, weight=None, cat_features=None, n=5):
    ...
    return _gower_topn_heap(
        ...
        weight_cat, weight_num, weight_sum,
        num_ranges, n, y_n_rows
    )


def _gower_topn_heap(query_cat, query_num, data_cat, data_num,
                     weight_cat, weight_num, weight_sum,
                     num_ranges, n, total_rows):
    """
...
def _compute_single_distance(query_cat, query_num, row_cat, row_num,
                            weight_cat, weight_num, weight_sum, num_ranges):
    """
    ...
    if len(query_num) > 0:
        ...
        normalized_delta = np.divide(abs_delta, num_ranges,
                                    out=np.zeros_like(abs_delta),
                                    where=num_ranges != 0)
