...
class TestSklearnIntegration:
    ...
    def test_gower_distance_function(self):
        ...
        dist_matrix = gower_distance(self.X_small, feature_weights=None, 
                                   cat_features=self.cat_features)
        assert dist_matrix.shape == (3, 3)
        assert np.all(np.diag(dist_matrix) == 0)
        
        # Test distance between two datasets
        dist_cross = gower_distance(self.X_small[:2], self.X_small[1:], 
                                  cat_features=self.cat_features)
    ...
    def test_precomputed_gower_matrix(self):
        ...
        assert distances_full['test'].shape == (2, 3)  # 2 test samples x 3 train samples
    ...
    def test_dbscan_clustering(self):
        ...
        X_cluster[:, 1] = np.random.choice(['A', 'B', 'C'], 20)  # Make middle column categorical
...
class TestSklearnNotAvailable:
    ...
    @pytest.mark.skipif(SKLEARN_AVAILABLE, reason="This test only runs when sklearn is NOT available")
